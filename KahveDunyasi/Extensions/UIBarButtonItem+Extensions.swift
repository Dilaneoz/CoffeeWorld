...
public class BadgeBarButtonItem: UIBarButtonItem { // sepete eklediğimiz ürünlerle ilgili sadece sepetin üzerinde değil bize bi bildirim geldiği zaman da ikonun üzerinde sayıları gösteren şeye badge denir
  
  private(set) lazy var badgeLabel: UILabel = { // butonun üzerindeki label(kullanıcı sepete kaç ürün eklediyse o kadar sayı gözükecek)
    let label = UILabel()
...
public extension BadgeBarButtonItem { // badge butonundaki text için bi extension yazıyoruz
  
  convenience init(button: UIButton, badgeText: String? = nil, target: AnyObject?, action: Selector) { // convenience init boş constructorda da eğer hiçbi değer girilmiyosa mutlaka parametre alan constructor a git diyoruz burada. boş constroctur da gelse şurdaki parametreleri alan(button: UIButton, badgeText: String? = nil, target: AnyObject?, action: Selector) constructor lara gitmesini söylüyoruz
...
public extension BadgeBarButtonItem { // badge butonundaki image için bi extension yazıyoruz
